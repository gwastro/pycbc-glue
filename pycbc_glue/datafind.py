...
>>> from pycbc_glue.datafind import GWDataFindHTTPConnection
...
By default, the returned L{Cache<pycbc_glue.lal.Cache>} object includes both C{gsiftp} and local C{file} versions of each frame, but the C{urlfile} keyword argument can be given to return only one of those:
...
from pycbc_glue import (lal, git_version, segments)
