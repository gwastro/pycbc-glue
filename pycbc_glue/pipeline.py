...
class LsyncCache:
  ...
  def get_lfns(self, site, frameType, gpsStart, gpsEnd):
    ...
    search = pycbc_glue.segments.segment(gpsStart, gpsEnd)

    # segment list representing the search interval
    searchlist = pycbc_glue.segments.segmentlist([search])
    ...
    for key,seglist in cache['gwf'][site][frameType].items():
      ...
      for s in seglist:
        if s.intersects(search):
          ...
          for t in times:
            if search.intersects(pycbc_glue.segments.segment(t, t + dur)):
